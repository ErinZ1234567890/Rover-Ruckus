 ...
 */
@Autonomous(name = "Fun CRATOR color testing*", group = "Sensor")
//@Disabled                            // Comment this out to add to the opmode list
public class cratorColorFun extends LinearOpMode {                              //;_;
...
//        lift.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        ...
        while (opModeIsActive()) {  //************ --> Starting
            ...
            while(runOnce) {  //only for the sensing part
                encodeLift(30, 6); //let's not drill

                encoderDrive(100,7.85,7.85,7.85,7.85); // moves to first mineral
                colorSense();

                mineralMove(orientation);

                encoderDrive(25,.5,.5,.5,.5); //just to ensure it gets into the crator
